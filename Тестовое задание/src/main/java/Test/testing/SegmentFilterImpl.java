...
public class SegmentFilterImpl implements SegmentFilter {
    ...
    public void showAllSegments(List<Flight> flights) {
        for (Flight flight : flights ) {
            ...
            for(int i = 0; i < flight.getSegments().size(); i++) {
                System.out.println(flight.getSegments().get(i) + " Segment " + flight.getSegments().get(i).getId());
            }
        }
    }
    ...
    public Set<Flight> getArrivalDateLessDepartureDate(List<Flight> flights) {
        ...
        for (Flight flight : flights ) {
            ...
            while (split.size() > 0) {
                LocalDateTime departureTime = split.get(0).getDateOfDeparture();
        ...
        for (Flight flight : flights ) {
            ...
            if (split.size() > 2) {
                while (split.size() > 2) {
                    ...
                    LocalDateTime departureTime = split.remove(1).getDateOfDeparture();
